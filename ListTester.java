...
public class ListTester {
	...
	private enum Result {
							IndexOutOfBounds, IllegalState, NoSuchElement, ConcurrentModification, UnsupportedOperation,
							NoException, UnexpectedException, True, False, Pass, Fail, MatchingValue, ValidString
	};
	...
	private void printSectionSummary() {
		...
		secFails = failures;
		System.out.printf(
		                  "Tests Run So Far: %d,  Passed: %d (%.1f%%),  Failed: %d\n",
		                      totalRun,
		                      passes,
		                      passes * 100.0
		                              / totalRun,
	...
	private void runTests() {
		...
		testEmptyList(newList, "newList"); // OG
		testSingleElementList(emptyList_addToFrontA_A, "emptyList_addToFrontA_A", LIST_A, STRING_A); // OG
		testTwoElementList(A_addToFrontB_BA, "A_addToFrontB_BA", LIST_BA, STRING_BA); // OG
		...
		testThreeElementList(ABC_set1D_DBC, "ABC_set1D_DBC", LIST_DBC, STRING_DBC);
	 ...
	 * 
	 * @return [C] after iteratorRemoveAfterNextC
	 */
	 ...
	 * 
	 * @return [A,D,C] after set1D
	 */
	private IndexedUnsortedList<Integer> ABC_set1D_DBC() {
		IndexedUnsortedList<Integer> list = AB_addToRearC_ABC();
		list.set(1, ELEMENT_D);
		return list;
	}

	private Scenario<Integer> ABC_set1D_DBC = () -> ABC_set1D_DBC();

	/**
	 * Scenario: [A,B,C] -> set(2,D) -> [A,B,D]
	 * 
	 * @return [A,B,D] after set2D
	 */
	...
	/**
	 * Scenario: [A,B] -> add0C -> [C,A,B]
	 * 
	 * @return [C,A,B] after add0C
	 */
	...
	private void testSingleElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents,
		...
		try {
			...
			printTest(scenarioName + "_testAddToRear", testAddToRear(scenario.build(), ELEMENT_X, Result.NoException));
			printTest(
			          scenarioName + "_testAddAfter"
			          + contentsString.charAt(0),
			              testAddAfter(scenario.build(), contents[0], ELEMENT_B, Result.NoException));
			printTest(
			          scenarioName + "_testAddAfterX",
			              testAddAfter(scenario.build(), ELEMENT_X, ELEMENT_Z, Result.NoSuchElement));
			printTest(
			...
			printTest(
			          scenarioName + "_testIterRemove",
			              testIterRemove(scenario.build().iterator(), Result.NoException));
			printTest(
			          scenarioName + "_iterNext_testIterHasNext",
			              testIterHasNext(iterAfterNext(scenario.build(), 1), Result.False));
			printTest(
			          scenarioName + "_iterNext_testIterNext",
			              testIterNext(iterAfterNext(scenario.build(), 1), null, Result.NoSuchElement));
			printTest(
			          scenarioName + "_iterNext_testIterRemove",
			              testIterRemove(iterAfterNext(scenario.build(), 1), Result.NoException));
			printTest(
			          scenarioName + "_iterNextRemove_testIterHasNext",
			              testIterHasNext(iterAfterRemove(iterAfterNext(scenario.build(), 1)), Result.False));
			printTest(
			          scenarioName + "_iterNextRemove_testIterNext",
			              testIterNext(
			                           iterAfterRemove(iterAfterNext(scenario.build(), 1)),
			                               null,
			                               Result.NoSuchElement));
			printTest(
			          scenarioName + "_iterNextRemove_testIterRemove",
	...
	private void testTwoElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents,
		...
		try {
			...
			printTest(
			          scenarioName + "_testAddToFront",
			              testAddToFront(scenario.build(), ELEMENT_B, Result.NoException));
			...
			printTest(
			          scenarioName + "_iterNextNext_testIterHasNext",
			              testIterHasNext((iterAfterNext(scenario.build(), 2)), Result.False));
			printTest(
			          scenarioName + "_iterNextNext_testIterNext",
			              testIterNext((iterAfterNext(scenario.build(), 2)), contents[0], Result.NoSuchElement));
			printTest(
			          scenarioName + "_iterNextNext_testIterRemove",
			              testIterRemove(iterAfterNext(scenario.build(), 2), Result.NoException));

			printTest(
			          scenarioName + "_iterNextNextRemove_testIterHasNext",
			              testIterHasNext(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.False));
			printTest(
			          scenarioName + "_iterNextNextRemove_testIterNext",
			              testIterNext(
			                           iterAfterRemove(iterAfterNext(scenario.build(), 2)),
			                               null,
			                               Result.NoSuchElement));
			printTest(
			          scenarioName + "_iterNextNextRemove_testIterRemove",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));

			printTest(
			          scenarioName + "_iterNextRemoveNext_testIterHasNext",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(
			          scenarioName + "_iterNextRemoveNext_testIterNext",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.NoSuchElement));
			printTest(
			          scenarioName + "_iterNextRemoveNext_testIterRemove",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			// ListIterator
			if (SUPPORTS_LIST_ITERATOR) {
				// TODO: will add for double-linked list
	...
	private void testThreeElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents,
		...
		try {
			...
			printTest(
			          scenarioName + "_iterNextNext_testIterNext",
			              testIterNext((iterAfterNext(scenario.build(), 2)), contents[0], Result.MatchingValue));
			printTest(
			          scenarioName + "_iterNextNext_testIterRemove",
			              testIterRemove(iterAfterNext(scenario.build(), 2), Result.NoException));
			printTest(
			...
			printTest(
			          scenarioName + "_iterNextRemoveNext_testIterNext",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.MatchingValue));
			printTest(
			          scenarioName + "_iterNextRemoveNext_testIterRemove",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.NoException));

			printTest(
			          scenarioName + "_iterNextNextRemoveNext_testIterHasNext",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(
			          scenarioName + "_iterNextNextRemoveNext_testIterNext",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(
			          scenarioName + "_iterNextNextRemoveNext_testIterRemove",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));

			printTest(
			          scenarioName + "_iterNextNextNext_testIterHasNext",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(
			          scenarioName + "_iterNextNextNext_testIterNext",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(
			          scenarioName + "_iterNextNextNext_testIterRemove",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));

			printTest(
			          scenarioName + "_iterNextNextNextRemove_testIterHasNext",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(
			          scenarioName + "_iterNextNextNextRemove_testIterNext",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(
			          scenarioName + "_iterNextNextNextRemove_testIterRemove",
			              testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
	...
	private void test_IterConcurrency() {
		...
		try {
			...
			list.removeFirst();
			printTest(
			          "A_removeFirst_testIterNextConcurrent",
			...
			list.removeLast();
			printTest(
			          "A_removeLast_testIterNextConcurrent",
			              testIterNext(it1, ELEMENT_A, Result.ConcurrentModification));
			list = emptyList_addToFrontA_A();
			it1 = list.iterator();
			list.removeLast();
			...
			list.addToFront(ELEMENT_B);
			printTest(
			          "A_addToFrontB_testIterNextConcurrent",
			...
			list.addToRear(ELEMENT_B);
			printTest(
			          "A_addToRearB_testIterNextConcurrent",
			              testIterNext(it1, ELEMENT_A, Result.ConcurrentModification));
			list = emptyList_addToFrontA_A();
			it1 = list.iterator();
			list.addToRear(ELEMENT_B);
			...
			list.addAfter(ELEMENT_B, ELEMENT_A);
			printTest(
			          "A_addAfterAB_testIterNextConcurrent",
			              testIterNext(it1, ELEMENT_A, Result.ConcurrentModification));
			list = emptyList_addToFrontA_A();
			it1 = list.iterator();
			list.addAfter(ELEMENT_B, ELEMENT_A);
